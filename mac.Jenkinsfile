...
try {
    stage 'start builder'
    withCredentials([[$class: 'StringBinding', credentialsId: 'caf0ec30-fcc7-40c4-9f71-a6f96fdbf511', variable: 'SLAVE_CREDS']]) {
        sh """#!/bin/bash -l
    ...
    node("browser-f-mac-builder") {
        stage 'checkout'
        checkout([$class: 'GitSCM', branches: [[name: COMMIT_ID]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'CheckoutOption', timeout: 100]], submoduleCfg: [], userRemoteConfigs: [[url: REPO_URL]]])
