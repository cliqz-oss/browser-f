...
%archivator_exe% l dist\install\sea\ghostery-%ff_exe%.%platform_prefix%.installer%STUB_PREFIX%.exe
%archivator_exe% x -opkg%STUB_PREFIX%_%lang% -y dist\install\sea\ghostery-%ff_exe%.%platform_prefix%.installer%STUB_PREFIX%.exe
...
cd ..
copy /b ..\mozilla-release\other-licenses\7zstub\firefox\7zSD.sfx + browser\installer\windows\instgen\app.tag + pkg_%lang%\installer.7z dist\install\sea\ghostery-%ff_exe%.%platform_prefix%.installer.exe
goto sign_installer
...
cd ..
copy /b ..\mozilla-release\other-licenses\7zstub\firefox\7zSD.sfx + browser\installer\windows\instgen\stub.tag + pkg%STUB_PREFIX%_%lang%\stub.7z dist\install\sea\ghostery-%ff_exe%.%platform_prefix%.installer-stub.exe

:sign_installer
"%CLZ_SIGNTOOL_PATH%" sign /t %timestamp_server_sha1% /f %CLZ_CERTIFICATE_PATH% /p %CLZ_CERTIFICATE_PWD% dist\install\sea\ghostery-%ff_exe%.%platform_prefix%.installer%STUB_PREFIX%.exe
"%CLZ_SIGNTOOL_PATH%" sign /fd sha256 /tr %timestamp_server_sha256% /td sha256 /f %CLZ_CERTIFICATE_PATH% /p %CLZ_CERTIFICATE_PWD% /as dist\install\sea\ghostery-%ff_exe%.%platform_prefix%.installer%STUB_PREFIX%.exe
"%CLZ_SIGNTOOL_PATH%" verify /pa dist\install\sea\ghostery-%ff_exe%.%platform_prefix%.installer%STUB_PREFIX%.exe
if ERRORLEVEL 1 (goto :error)
