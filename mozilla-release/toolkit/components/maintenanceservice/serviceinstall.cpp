...
#define MAINT_UNINSTALL_KEY L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\GhosteryMaintenanceService"
                              ...
                              MaintenanceServiceStringTable *results)
{
  ...
  const char *kServiceKeys = "GhosteryMaintenanceDescription\0";
...
/**
 * Determines if the GhosteryMaintenance service path needs to be updated
...
{
  // When we originally upgraded the GhosteryMaintenance service we
  ...
  if (doesServiceHaveCorrectPath) {
    LOG(("The GhosteryMaintenance service path is correct."));
    servicePathWasWrong = FALSE;
    return TRUE;
  }
  // This is a recoverable situation so not logging as a warning
  LOG(("The GhosteryMaintenance path is NOT correct. It was: %ls",
       currentServicePath));
