...
var gPrivacyPane = {
  ...
  readBlockCookies() {
    let pref = Services.cookies.cookieBehavior;

    if (Ci.nsICookieService.BEHAVIOR_ACCEPT == pref) {
  ...
  writeBlockCookies(value) {
    switch (value) {
      ...
      case "disallow":
        let pref = Services.cookies.cookieBehavior;
        let blockCookiesMenuValue = pref == Ci.nsICookieService.BEHAVIOR_ACCEPT ?
          "trackers" :
  ...
  readBlockCookiesMenu() {
    ...
    this.toggleCookieExpirationControlState(
      !(pref == Ci.nsICookieService.BEHAVIOR_REJECT));
  ...
  writeBlockCookiesMenu(value) {
    switch (value) {
      ...
      case "trackers-plus-isolate":
        return Ci.nsICookieService.BEHAVIOR_REJECT_TRACKER_AND_PARTITION_FOREIGN;
    }
    return Ci.nsICookieService.BEHAVIOR_ACCEPT;
  },
